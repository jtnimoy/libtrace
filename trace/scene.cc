...
{
 cout << "Rendering " << image.get_width() << "*" << image.get_height() << ":\n"; 
 image.allocate();
 ...
 for (y=0; y<get_yres(); y++)
 {
  ...
  if (y>0)
  {
   ...
   cout << " \tTime remaining: " << w << "w " << d << "d " << h << "h" << m << "m" << s << "s";
  }
...
{
#warning TODO: make DOF effects adaptive if full scene AA is off
/*
